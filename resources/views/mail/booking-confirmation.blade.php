<x-mail-layout>
    ...
    <p style="Margin:0;mso-line-height-rule:exactly;font-family:helvetica, 'helvetica neue', arial, verdana, sans-serif;line-height:32px !important;letter-spacing:0;color:#333333;font-size:16px"
        class="es-text-mobile-size-16">
        {{ __('ui.mailDepositMessage') }}
    </p>

    <p style="Margin:0;mso-line-height-rule:exactly;font-family:helvetica, 'helvetica neue', arial, verdana, sans-serif;line-height:32px !important;letter-spacing:0;color:#333333;font-size:16px"
        class="es-text-mobile-size-16">

        {!! __('ui.paymentAmountMessage', ['amount' => number_format($booking->bookingData['price'], 2) * 0.3, 'code' => $booking->code]) !!}
    ...
    <p
        ...
        <a
            href="{{ route('booking.status', ['locale' => $booking->locale, 'code' => $booking->code, 'email' => $booking->email]) }}">{{ __('ui.clickHere') }}</a>​
    </p>
</x-mail-layout>
